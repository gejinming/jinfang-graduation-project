 ...
 */
@RestController
@RequestMapping("/teacher/headman/document")
@AllArgsConstructor
@Api(tags = "论文及资料下载接口", description = "答辩组组长..")
public class TeacherHeadmanDocumentController extends BaseController {

    private GpDissertationService gpDissertationService;
    private BornDocumentService bornDocumentService;

    //    @PreAuthorize("hasAuthority('teacher:headman:document:list')")
    @ApiOperation("分页查询列表")
    ...
    public HttpResult findPage(@RequestBody PageRequest pageRequest) {
        ...
        return HttpResult.ok(gpDissertationService.findPageInPassedByHeadman(pageRequest));
    }

    //    @PreAuthorize("hasAuthority('teacher:headman:document:download')")
